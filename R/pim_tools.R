...
pim_read<-function(file){
    ...
    metadataout <- data.frame('SamplingUnit' = outmsu, 'Date' = outmdate, 'Assessor'= outmass, 'TransectNo' = outmtrans, stringsAsFactors=F)
    ...
    datout <- data.frame('Date' = outdate, 'Assessor' = outass, 'SamplingUnit' = outsu, 'Transect' = outtrans, 'Step' = outsteps, 'FieldName' = outoldspp, 'ScientificName' = outnewspp, 'Strata' = outs, 'Condition' = outc, stringsAsFactors=F)
